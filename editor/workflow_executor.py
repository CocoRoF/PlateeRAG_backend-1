...
class WorkflowExecutor:
    ...
    async def stream_workflow(self) -> AsyncGenerator[Dict[str, Any], None]:
        """워크플로우를 비동기적으로 실행하고 결과를 스트리밍합니다."""
        self._build_graph()
        execution_order: List[str] = self._topological_sort()

        node_outputs: Dict[str, Dict[str, Any]] = {}

        print("--- 스트리밍 워크플로우 실행 시작 ---")
        print(f"실행 순서: {' -> '.join(execution_order)}")
        ...
        for node_id in execution_order:
            ...
            for edge in connected_edges:
                ...
                if source_node_id in node_outputs and source_port_id in node_outputs[source_node_id]:
                    value = node_outputs[source_node_id][source_port_id]
                    if target_port_id not in port_values: port_values[target_port_id] = []
                    port_values[target_port_id].append(value)
            for port_id, values in port_values.items():
                kwargs[port_id] = values[0] if len(values) == 1 else values
            if 'parameters' in node_info['data'] and node_info['data']['parameters']:
                for param in node_info['data']['parameters']:
                    param_key: Optional[str] = param.get('id')
                    param_value: Any = param.get('value')
                    if param_key and param_key not in kwargs: kwargs[param_key] = param_value
            
            print(f"\n[스트림 실행] {node_info['data']['nodeName']} ({node_id})")

            instance = NodeClass()
            
            is_streaming_node = hasattr(instance, 'astream') and callable(instance.astream)

            try:
                if is_streaming_node:
                    yield {"type": "stream_start", "node_id": node_id}
                    final_response = ""
                    async for chunk in instance.astream(**kwargs):
                        token = chunk if isinstance(chunk, str) else str(chunk)
                        final_response += token
                        yield {"type": "token", "node_id": node_id, "data": token}
                    
                    result = final_response
                    yield {"type": "stream_end", "node_id": node_id, "final_data": result}
                else:
                    yield {"type": "node_start", "node_id": node_id}
                    result = await asyncio.to_thread(instance.execute, **kwargs)
                    yield {"type": "node_end", "node_id": node_id, "data": result}

            except Exception as e:
                logger.error("Error executing node %s in stream: %s", node_id, str(e), exc_info=True)
                yield {"type": "error", "node_id": node_id, "message": str(e)}
                raise

            function_id: str = node_info['data']['functionId']
            if function_id != 'endnode' and node_info['data']['outputs']:
                output_port_id: str = node_info['data']['outputs'][0]['id']
                node_outputs[node_id] = {output_port_id: result}
                print(f" -> 스트림 출력: {str(result)[:50]}...")
